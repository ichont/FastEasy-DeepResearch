...
class ReportStructureNode(StateMutationNode):
    ...
    def process_output(self, output: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.log_error(f"处理输出失败，转为默认结构: {str(e)}")
            # 返回更丰富的默认结构
            return [
                {
                    "title": "引言和背景介绍",
                    "content": f"对'{self.query}'的背景、重要性和研究意义进行介绍"
                },
                {
                    "title": "核心问题分析", 
                    "content": f"深入分析'{self.query}'涉及的核心问题和关键要素"
                },
                {
                    "title": "关键发现或数据",
                    "content": f"展示与'{self.query}'相关的重要发现、统计数据或案例分析"
                },
                {
                    "title": "影响和意义",
                    "content": f"探讨'{self.query}'带来的影响、启示和长远意义"
                },
                {
                    "title": "总结和展望",
                    "content": f"对'{self.query}'进行总结，并对未来发展趋势进行展望"
                }
            ]
